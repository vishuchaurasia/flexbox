...
# ---------------------------------------------------------------------------
# FlexBox seed schema (creates 50 products, many categories, variants)
# ---------------------------------------------------------------------------

FLEXBOX_SCHEMA = {
    "Smartphones": {
        "category": {"name": "Mobiles", "image_name": "mobiles.svg"},
        "product_attributes": {"Brand": ["FlexBox", "Nova", "Zen"], "Storage": ["64GB", "128GB", "256GB"]},
        "variant_titles": ["Black", "Blue", "Green"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Laptops": {
        "category": {"name": "Laptops", "image_name": "laptops.svg"},
        "product_attributes": {"Brand": ["FlexBox", "Aero", "ProLite"], "RAM": ["8GB", "16GB"], "SSD": ["256GB", "512GB"]},
        "variant_titles": ["8GB / 256GB", "16GB / 512GB"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Headphones": {
        "category": {"name": "Electronics", "image_name": "electronics.svg"},
        "product_attributes": {"Type": ["Wireless", "Wired"], "Brand": ["FlexBox", "Pulse"]},
        "variant_titles": ["Black", "White"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Men's Fashion": {
        "category": {"name": "Men", "image_name": "fashion.svg"},
        "product_attributes": {"Brand": ["FlexBox", "Urban"], "Fit": ["Regular", "Slim"]},
        "variant_titles": ["S", "M", "L", "XL"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Women's Fashion": {
        "category": {"name": "Women", "image_name": "fashion.svg"},
        "product_attributes": {"Brand": ["FlexBox", "Aura"], "Style": ["Casual", "Ethnic"]},
        "variant_titles": ["S", "M", "L", "XL"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Home Decor": {
        "category": {"name": "Home & Living", "image_name": "home.svg"},
        "product_attributes": {"Material": ["Wood", "Metal", "Glass"], "Brand": ["FlexBox"]},
        "variant_titles": ["Small", "Medium", "Large"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Kitchen": {
        "category": {"name": "Appliances", "image_name": "appliances.svg"},
        "product_attributes": {"Power": ["700W", "900W", "1200W"], "Brand": ["FlexBox"]},
        "variant_titles": ["Standard", "Plus"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Beauty": {
        "category": {"name": "Beauty", "image_name": "beauty.svg"},
        "product_attributes": {"Type": ["Skincare", "Grooming"], "Brand": ["FlexBox", "Glow"]},
        "variant_titles": ["Pack of 1", "Pack of 2"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Sports": {
        "category": {"name": "Sports", "image_name": "sports.svg"},
        "product_attributes": {"Brand": ["FlexBox", "Sprint"], "Level": ["Beginner", "Pro"]},
        "variant_titles": ["Standard", "Pro"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
    "Books": {
        "category": {"name": "Books", "image_name": "books.svg"},
        "product_attributes": {"Language": ["English", "Hindi"], "Format": ["Paperback", "Hardcover"]},
        "variant_titles": ["Paperback", "Hardcover"],
        "images_dir": "products/",
        "is_shipping_required": True,
    },
}
...
FLEXBOX_COLLECTIONS_SCHEMA = [
    {"name": "FlexBox Top Deals", "image_name": "deals.svg"},
    {"name": "FlexBox New Arrivals", "image_name": "new.svg"},
]
...
def create_admin():
    ...
    admin_user = User.query.filter_by(email="visaladmin121@test.com").first()
    if not admin_user:
        admin_user = User.create(
            username="visaladmin121", 
            email="visaladmin121@test.com", 
            password="vishal@123", 
            is_active=True
        )
        create_fake_address(admin_user.id)
        create_fake_address(admin_user.id)
        create_fake_address(admin_user.id)
        UserRole.create(user_id=admin_user.id, role_id=4)
        yield f"✓ Admin user created: visaladmin121@test.com"
    else:
        yield f"✓ Admin user already exists: visaladmin121@test.com"
    
    # Create regular user with custom credentials
    regular_user = User.query.filter_by(email="visal121@test.com").first()
    if not regular_user:
        ...
        create_fake_address(regular_user.id)
        yield f"✓ Regular user created: visal121@test.com"
    else:
        yield f"✓ Regular user already exists: visal121@test.com"
    
    # Keep old test admin users for compatibility
    old_admin = User.query.filter_by(username="admin").first()
    if not old_admin:
        old_admin = User.create(
            username="admin", email="admin@163.com", password="admin", is_active=True
        )
        UserRole.create(user_id=old_admin.id, role_id=4)
        yield f"✓ Legacy admin user created: admin@163.com"
    else:
        yield f"✓ Legacy admin user already exists: admin@163.com"
...
def create_page():
    content = """
    <h2 align="center">Welcome to FlexBox</h2>
    <h3 align="center">A modern, fast e-commerce experience built with Flask</h3>
    <p style="text-align:center;margin-top:16px;">
        <img alt="FlexBox" style="max-width:520px;width:100%;height:auto;"
             src="/static/placeholders/flexbox/hero.svg">
    </p>
...
def create_menus():
    ...
    item, _ = MenuItem.get_or_create(title="FlexBox", position=2)
